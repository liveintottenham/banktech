...
# 네비게이션 바
def render_nav():
    current_page = st.query_params.get("page", ["home"])[0] if st.query_params.get("page") else "home"
    
    st.markdown(f"""
    <div class="nav-container">
        <a class="nav-item {'active' if current_page == 'home' else ''}" href="?page=home">{get_text('home')}</a>
        <a class="nav-item {'active' if current_page == 'loan' else ''}" href="?page=loan">{get_text('loan')}</a>
        <a class="nav-item {'active' if current_page == 'payroll' else ''}" href="?page=payroll">{get_text('payroll')}</a>
    </div>
    """, unsafe_allow_html=True)
...
def render_dashboard():
    ...
    <div class="dashboard-header">
        <div class="welcome-section">
            <div>
                <h2 style="margin:0">{get_text('welcome').format(st.session_state.user_data['name'])}</h2>
                <p style="color:#666">{st.session_state.user_data['department']} | {get_text('last_login')}: {datetime.now().strftime('%Y/%m/%d %H:%M')}</p>
            </div>
...
def show_payroll():
    ...
    with st.form("payslip_form"):
        ...
        if st.form_submit_button(get_text('create_payslip'), use_container_width=True):
            ...
            # None 값이 있는 항목 제거
            income_items = [item for item in income_items if item['amount'] is not None and item['amount'] > 0]
            deduction_items = [item for item in deduction_items if item['amount'] is not None]
            ...
            <div class="paystub-container">
                <div class="paystub-header">
                    ...
                    <div style="display:flex; justify-content:center; gap:3rem; color:#666; margin-top:1rem">
                        <div>社員番号: {st.session_state.user_data["emp_num"]}</div>
                        <div>発行日: {datetime.now().strftime('%Y/%m/%d')}</div>
                        <div>支給日: {date.today().replace(day=25).strftime('%Y/%m/%d')}</div>
                    </div>
                </div>

                <div class="section-title">🔼 {get_text('income_breakdown')}</div>
                {income_rows}
                <div class="amount-row total-row">
                    <span>{get_text('total_income')}</span>
                    <span>¥{total_income:,}</span>
                </div>

                <div class="section-title">🔽 {get_text('deduction_breakdown')}</div>
                {deduction_rows}
                <div class="amount-row total-row">
                    <span>{get_text('total_deduction')}</span>
                    <span>¥{total_deduction:,}</span>
                </div>

                <div class="amount-row net-pay-row">
                    <span>{get_text('net_pay')}</span>
                    <span>¥{net_pay:,}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def loan_management():
    ...
    with st.expander("📝 積立口座新規登録", expanded=True):
        with st.form("savings_form"):
            ...
            name = cols[0].text_input(get_text('customer_name'), value=st.session_state.savings_data['name'])
            emp_num = cols[1].text_input(get_text('employee_number'), value=st.session_state.savings_data['emp_num'])
            account = cols[2].text_input(get_text('account_number'), value=st.session_state.savings_data['account'])
            start_date = cols[3].date_input(get_text('start_date'), value=st.session_state.savings_data['start_date'])
            
            cols2 = st.columns([1,1,1,1])
            unit_price = cols2[0].number_input(get_text('unit_price'), value=st.session_state.savings_data['unit_price'], min_value=1000)
            units = cols2[1].number_input(get_text('units'), value=st.session_state.savings_data['current_units'], min_value=1)
            years = cols2[2].selectbox(get_text('years'), [1,2,3,5], index=2)
            interest = cols2[3].number_input(get_text('interest_rate'), value=st.session_state.savings_data['interest'], min_value=0.0)
    ...
    <div class="metric-grid">
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">¥{calc['total_interest']:,.0f}</div>
        </div>
        <div class="metric-card">
