...
# CSS 스타일링 - 완전히 개선된 버전
def load_css():
    # CSS가 이미 로드되었는지 확인하고, 로드되지 않았을 때만 적용
...
def main_layout():
    ...
    # 상단 컨트롤을 Streamlit 컬럼으로 완전히 재구성
    ...
    with st.container():
        ...
        with col1:
            ...
            if st.button("🌐 English / 英語", key="lang_switcher", use_container_width=True,
        ...
        with col2:
            # 상태 표시기
            st.markdown(f"""
            <div class="control-item status-online">
        ...
        with col4:
            ...
            if st.button(get_text('logout'), key="logout_btn", use_container_width=True, type="secondary"):
                st.session_state.logged_in = False
                st.session_state.css_loaded = False  # CSS 상태도 리셋
                st.rerun()
...
def show_announcement():
    announcements = [
        {
            ...
            "content": "12月25日 2:00-4:00 システムメンテナンスのため一時的にサービスがご利用いただけません / System will be temporarily unavailable due to maintenance from 2:00-4:00 on Dec 25th",
            "date": "2024-12-20",
            "priority": "high"
        },
        {
            ...
            "content": "高金利プランのご案内。年利3.5%の新しい積立プランが利用可能です / New high-interest savings plan available with 3.5% annual interest rate",
            "date": "2024-12-18",
            "priority": "medium"
        },
        {
            ...
            "content": "12月29日～1月4日まで休業となります。ご不便をおかけしますが何卒ご了承ください / Closed from Dec 29 to Jan 4. We apologize for any inconvenience",
            "date": "2024-12-15",
            "priority": "medium"
        }
    ]
    ...
    for announcement in announcements:
        badge_class = {
            ...
            "medium": "badge-medium", 
            "low": "badge-low"
        }.get(announcement["priority"], "badge-medium")
        
        badge_text = {
            "high": "重要 / Important",
            "medium": "情報 / Information", 
            "low": "お知らせ / Notice"
        }.get(announcement["priority"], "情報 / Information")
        ...
        <div class="announcement-item">
            ...
            <div class="announcement-badge {badge_class}">{badge_text}</div>
        </div>
        """, unsafe_allow_html=True)
